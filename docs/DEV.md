...
- `races/{race_team_station_round}` — סבב: `team, station, round, status, startedAt, startedBy, endedAt?, endedBy?, endedReason?, timeLimitSeconds`, צילום הרשימה `participantIds`, ו-`tags: []` לציונים/הערות/מדידות שאינן סדר הגעה. הערך החדש הוא 2400; בקריאת סבבים ישנים ללא השדה משתמשים באותה ברירת מחדל.
...
- זמן סימון נכתב כ-`serverTimestamp` לפי המקום. עד 40 דקות מ-`startedAt` הוא מוצג ונשלח; לאחר מכן המקום נשמר, אך הזמן מוצג כ"ללא זמן" ונשלח כתא ריק, לא `0`. ספירה בלחיצה נעצרת בתקרה; סדר הגעה, הערכה והזנה ידנית נשארים ניתנים לתיקון.
...
3. **Code.gs** — עורכים ב-[script.google.com](https://script.google.com) → מדביקים את `scripts/Code.gs` →
...
4. **Firebase** — קונפיג ב-`firebase-config.js`. מפתחות ה-client ציבוריים (אבטחה דרך Firestore rules + Authorized domains).
...
npm test
```
`test:unit` בודק את מדיניות הזמן וסכימת הדיווח. `test:rules` מעלה Firestore Emulator זמני ובודק שמפק״צ בלבד שולט בסבב, שמעריך עדיין יכול להעריך אחרי עצירה, ושדיווחי תקלות אינם חושפים מידע או ניתנים לעריכת המדווח.
