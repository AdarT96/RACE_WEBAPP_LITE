...
- המפק״צ תמיד נשאר בסבב האחרון. מעריך רשאי לעבור בין הטאבים; ציונים/הערות ניתנים לעריכה בסבב היסטורי. מדידה בלחיצה וסדר הגעה נעולים אחרי עצירה; מדידה ידנית מיועדת להשלמה ותיקון גם בסיום התחנה.
...
מנהל: הגדר צוות (משתתפים + סדר תחנות) → מפק״צ: ▶ התחל → שני מעריכים: ודאו שהזמן התחיל, סמנו תוצאות/ציונים/הערות במקביל → ודאו שבמובייל גריד הפעולה מתאים את עצמו למסך לאורך (10 משתתפים = 3 עמודות ו־4 שורות; 15 משתתפים = 3×5), ושגודל הכפתורים לא משתנה כשמסמנים הגעה → מפק״צ: ⏹ עצור →
בעצבים מברזל ודאו שאין כפתורי ספירה, בחרו משתתף ושמרו מספר ברגים גם אחרי עצירה → בספרינטים/אלונקה העבירו מעריך לסבב קודם וחזרה → **סנכרן ל-Sheets** → בדוק בתיקיית `גיבוש …` בקובץ הצוות.
